...
static void compute_mem_size(struct bootinfo_mem_map* mem_map) {
...
static void init_first_bitmap(void) {
...
static struct mem_bitmap* create_bitmap(size_t index) {
...
static void find_first_free_bitmap(void) {
...
static struct mem_bitmap* find_bitmap_by_index(size_t index) {
...
static bool_t try_set_page(addr_t paddr, bool_t state) {
  struct mem_bitmap* mem_bitmap;
  status_t sts;
  size_t bitmap, index;
...
bool_t pm_try_lock_pages(addr_t paddr, size_t bytes) {
  size_t pages;
...
bool_t pm_try_release_pages(addr_t paddr, size_t bytes) {
  size_t pages;
  bool_t no_collisions;
  if (bytes == 0)
