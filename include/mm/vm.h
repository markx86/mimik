...
#define vm_map_range(table, paddr_start, paddr_end, vaddr_hint, flags) \
  vm_map_pages(table, paddr_start, PAGES(paddr_end - paddr_start), vaddr_hint, flags)
...
#define vm_map_kernel_range(paddr_start, paddr_end, vaddr_hint, flags) \
  vm_map_kernel_pages(paddr_start, PAGES(paddr_end - paddr_start), vaddr_hint, flags)

void vm_unmap_pages(ptr_t table, addr_t vaddr_start, size_t pages);
#define vm_unmap_page(table, vaddr) \
...
#define vm_unmap_kernel_page(vaddr) \
  vm_unmap_kernel_pages(vaddr, 1)
