...
bool_t has_apic(struct bootinfo_arch* bootinfo) {
	/* TODO: check for APIC support */
	return FALSE;
}

status_t new_pic_init(struct bootinfo_arch* bootinfo) {
...
static void old_pic_remap(uint8_t off1, uint8_t off2) {
...
static inline void old_pic_mask(uint8_t irq) {
...
static inline void old_pic_unmask(uint8_t irq) {
	uint8_t value, port = irq < 8 ? PIC1_DATA : PIC2_DATA;
	irq &= 7;
	value = io_inb(port) & (uint8_t)~(1 << irq);
	io_outb(port, value);
...
static void old_pic_ack(uint8_t irq) {
	if (irq >= 8)
		io_outb(PIC2_COMM, PIC_EOI);
	io_outb(PIC1_COMM, PIC_EOI);
}

static status_t old_pic_init(void) {
